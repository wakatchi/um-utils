...
class ShortFunctionTest extends TestCase{
    ...
    function testCurrensyDisplayFormat() {
        {
            $expected = '0円';
            $result = wk_currensy_display_format(null);
            $this->assertEquals($expected, $result);
        }
        {
            $expected = '0円';
            $result = wk_currensy_display_format('');
            $this->assertEquals($expected, $result);
        }
        {
            $expected = '100円';
            $result = wk_currensy_display_format(100);
            $this->assertEquals($expected, $result);
        }
        {
            $expected = 'foo';
            $result = wk_currensy_display_format(null,$expected);
            $this->assertEquals($expected, $result);
        }
        {
            $expected = '1,000円';
        ...
        {
            $expected = '-1円';
            $result = wk_currensy_display_format(-1);
            $this->assertEquals($expected, $result);
        }
    }
    ...
    function testShortenCharacters() {
        ...
        {
            $str = str_repeat("a",50);
            $actual = wk_shorten_characters($str);
            $this->assertEquals($actual,$str);
        }
        // サイズ指定
        {
            $str = str_repeat("b",50);
            $actual = wk_shorten_characters($str,49);
    ...
    public function testDatetimeDisplayFormat_yyyyMM(){
        // 空
        {
            $actual = wk_datetime_display_format_yyyyMM(null);
            $this->assertEmpty($actual);
        }
        // 代替文字列指定
        {
            $actual = wk_datetime_display_format_yyyyMM(null,"foo");
            $this->assertEquals($actual,'foo');
        }
        // 通常変換
        {
            $now = '2020-03-01';
            $actual = wk_datetime_display_format_yyyyMM($now);
    ...
    public function testDatetimeDisplayFormat_yyyyMMdd(){
        // 空
        {
            $actual = wk_datetime_display_format_yyyyMMdd(null);
            $this->assertEmpty($actual);
        }
        // 代替文字列指定
        {
            $actual = wk_datetime_display_format_yyyyMMdd(null,"foo");
            $this->assertEquals($actual,'foo');
        }
        // 通常変換
        {
            $now = '2020-03-01';
            $actual = wk_datetime_display_format_yyyyMMdd($now);
    ...
    public function testDatetimeDisplayFormat_yyyyMMddhhmm(){
        // 空
        {
            $actual = wk_datetime_display_format_yyyyMMddhhmm(null);
            $this->assertEmpty($actual);
        }
        // 代替文字列指定
        {
            $actual = wk_datetime_display_format_yyyyMMddhhmm(null,"foo");
            $this->assertEquals($actual,'foo');
        }
        // 通常変換
        {
            $now = '2020-03-01 08:10';
            $actual = wk_datetime_display_format_yyyyMMddhhmm($now);
    ...
    public function testSafetyUnset(){
        ...
        // 可変引数で無効化
        {
            ...
            wk_safety_unset($actual,"foo","scott");
            $this->assertArrayNotHasKey("foo",$actual,"keyが削除されていない");
